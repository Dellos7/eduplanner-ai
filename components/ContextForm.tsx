...
import { ArrowRight, CheckSquare, Square, Clock, BookOpen, Users, Globe, Briefcase } from 'lucide-react';
...
const ContextForm: React.FC<ContextFormProps> = ({ initialData, onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 space-y-8 animate-fade-in">
      <div className="border-b border-slate-100 pb-4">
        <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
          <BookOpen className="w-6 h-6 text-indigo-600" />
          Datos Generales
        </h2>
        <p className="text-slate-500 mt-1">Información base para los documentos.</p>
      </div>
         ...
         <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 space-y-4">
            ...
            {formData.generateFullCourse && (
              <p className="text-xs text-indigo-600">
                La IA calculará el número óptimo de SAs basándose en el currículum y las {formData.weeklyHours || 'X'} horas semanales.
              </p>
            )}
         </div>
      </div>
